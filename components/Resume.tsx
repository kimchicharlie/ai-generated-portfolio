...
const Resume = (): React.JSX.Element => {
        ...
        try {
          ...
          return new Promise((resolve) => {
            img.onload = () => {
              ...
              console.log(
                `Added ${sectionName} at Y=${(currentY - imgHeight - 5).toFixed(
                  1
                )}mm`
              );
              resolve();
  ...
  // Keep the old function for backward compatibility (now uses PDF)
  const handleDownloadScreenshot = async (
    format: PageFormat
  ): Promise<void> => {
    return handleDownload(format, "pdf");
  };
